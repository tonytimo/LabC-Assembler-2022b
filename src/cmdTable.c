...
int addCmd(unsigned int decode, unsigned int destOperand,
           unsigned int srcOperand, unsigned int opcode) {
    cmdNode *current, *newNode;
    word newWord;
    ...
    if ((newNode = createCmdNode(newWord))
        ...
        if (cmd_list.head == NULL) {
            ...
            /* skip to the end of the list */
            for (current = cmd_list.head; current->next != NULL; current =current->next);
...
void printCmdTable(void) {
    ...
    for (curr = cmd_list.head; curr != NULL; curr = curr->next, lineNumber++) {
        printf("%s\t%s\n", convertDecimalTo32(lineNumber),
               convertDecimalTo32(curr->value.print));
    }
    printDataTable(lineNumber);
}
